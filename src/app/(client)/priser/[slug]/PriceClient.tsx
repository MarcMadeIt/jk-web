...
const PriceClient = ({ slug }: Props) => {
  ...
  const seoTitle = t(`productPage.${slug}.title`);
  const seoSubTitle = t(`productPage.${slug}.subtitle`);
  const seoHero = t(`productPage.${slug}.hero`);
  const seoBtn = t(`productPage.common.btn`);
  const cta = t(`productPage.${slug}.cta`);
